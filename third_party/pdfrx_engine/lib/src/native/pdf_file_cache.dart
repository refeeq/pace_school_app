...
Future<PdfDocument> pdfDocumentFromUri(
  ...
  final fileCache = cache ??= await PdfFileCache.fromUri(uri);
  final httpClientWrapper = _HttpClientWrapper(Pdfrx.createHttpClient ?? () => http.Client());

  try {
    if (!fileCache.isInitialized) {
      fileCache.setBlockSize(blockSize ?? PdfFileCache.defaultBlockSize);
      ...
      if (fileCache.cacheControlState.cacheControl.mustRevalidate &&
          fileCache.cacheControlState.isFresh(now: DateTime.now())) {
        ...
        if (result.isFullDownload || result.notModified) {
          fileCache.close(); // close the cache file before opening it.
    ...
    return await entryFunctions.openCustom(
      read: (buffer, position, size) async {
        ...
        for (var p = position; p < end;) {
          final blockId = p ~/ fileCache.blockSize;
          final isAvailable = fileCache.isCached(blockId);
          if (!isAvailable) {
          ...
          final readEnd = min(p + size, (blockId + 1) * fileCache.blockSize);
          final sizeToRead = readEnd - p;
          await fileCache.read(buffer, bufferPosition, p, sizeToRead);
          p += sizeToRead;
    ...
    if (e is PdfException && e.errorCode == pdfium_bindings.FPDF_ERR_FORMAT) {
      ...
      await fileCache.deleteCacheFile();
    }
    fileCache.close();
    httpClientWrapper.reset();
    rethrow;
